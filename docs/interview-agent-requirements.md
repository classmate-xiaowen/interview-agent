...
**FR-3.6 对话内容沉淀到知识库（一期补充）**
- 目标：让用户在与 Agent 对话中持续把有价值的面试题沉淀进知识库，知识库越丰富、后续 RAG 检索越精准，长期降低重复生成成本（「越用 token 越省」）。
- 单条导入：每条助手消息（面试官提出的问题，含追问）气泡下方提供「存入知识库」按钮；点击弹出编辑弹窗，预填问题文本、公司（继承本场 `target_company`）、心态标签（由 `interviewer_style` 映射：pressure→压力型 / gentle→温和引导型 / deep→技术深挖型），并允许编辑问题、公司、心态标签（多选）、难度（1-5）、结果（通过/未通过/待定）、备注；确认后写入 `InterviewRecord`（触发向量化）。导入成功后该条标记为「已存入知识库」，按钮置灰防重复。
- 批量导入：对话页工具栏提供「批量导入本场题目」按钮（带待导入数量徽标），一键把本场所有「未导入的面试题」按继承的元数据批量写入知识库（`POST /api/knowledge/import`），导入后统一标记。
- 范围：仅导入面试题回合（`InterviewTurn.is_summary = false`）；面试结束的「总结/整体点评」回合（`is_summary = true`）不导入，因其不是可检索的「题」。
- 去重：v1 仅前端会话内存态去重（按消息索引标记已导入），不跨刷新持久化、不在 DB 层强去重；用户如需去重/修订可在知识库页操作。
- 复用：完全复用既有 `POST /api/knowledge/records`（单条）与 `POST /api/knowledge/import`（批量）接口与 `knowledge_service` 写入链路，无新增后端接口。
